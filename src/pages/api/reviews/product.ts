...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (!productId) {
    return new Response(JSON.stringify({ error: 'product_id wajib diisi' }), { status: 400 });
  }
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!product_id || !store_id || !reviewer_name?.trim()) {
      return new Response(JSON.stringify({ error: 'product_id, store_id, dan nama wajib diisi' }), { status: 400 });
    }
    ...
    const { rows: products } = await query('SELECT id FROM products WHERE id = $1 AND store_id = $2', [product_id, store_id]);
    if (products.length === 0) {
      return new Response(JSON.stringify({ error: 'Produk tidak ditemukan' }), { status: 404 });
    }

    const { rows } = await query(
      'INSERT INTO product_reviews (product_id, store_id, reviewer_name, rating, comment) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [product_id, store_id, reviewer_name.trim(), ratingNum, comment?.trim() || null],
    );
